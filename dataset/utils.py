...
def transtrlong(ts):
    tstr = str(ts)
    return tstr[:4] + tstr[5:7] + tstr[8:10] + str((int(tstr[11:13]) * 2 + 0 if tstr[14:16] == '00' else 1) + 1).zfill(
        2)
...
if __name__ == '__main__':
    dtstr = '2016-03-10T00:00'
    datatime = np.datetime64(dtstr, 'm')
    # datatime_str = np.datetime_as_string(datatime)
    # datatime_arr = np.array(datatime)
    wday = time.strptime(str(datatime)[:10], '%Y-%m-%d')
    print(f'{wday[0]}-{wday[1]}-{wday[2]}')
    # print(wday, hour, halfhour)
